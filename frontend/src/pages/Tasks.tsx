...
const Tasks = () => {
  ...
  const priorityMeta: Record<string, { class: string; icon: LucideIcon }> = {
  ...
  const statusMeta: Record<string, { icon: LucideIcon; color: string; bgColor: string; label: string }> = {
  ...
  const counts: Record<string, number> = tasks.reduce<Record<string, number>>((acc, t) => {
