...
const getAllRoles = async (req, res) => {
  try {
    const [roles] = await pool.execute('SELECT * FROM roles ORDER BY role_id');
...
const approveUser = async (req, res) => {
  try {
    ...
    const [roles] = await pool.execute('SELECT role_id, role_name FROM roles WHERE role_id = ?', [role_id]);
    if (roles.length === 0) {
...
const rejectUser = async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM activity_logs WHERE user_id = ?', [id]);
...
const updateUser = async (req, res) => {
  try {
    ...
    const [existing] = await pool.execute('SELECT username FROM users WHERE user_id = ?', [id]);
...
const toggleUserActive = async (req, res) => {
  try {
    ...
    await pool.execute('UPDATE users SET is_active = NOT is_active WHERE user_id = ?', [id]);
...
const resetPassword = async (req, res) => {
  try {
    ...
    await pool.execute('UPDATE users SET password = ? WHERE user_id = ?', [hashed, id]);
...
const deleteUser = async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM tasks WHERE assigned_to = ? OR assigned_by = ?', [id, id]).catch(() => {});
    // Nullable FKs — just clear the reference
    await pool.execute('UPDATE stock_movements SET created_by = NULL WHERE created_by = ?', [id]).catch(() => {});
    await pool.execute('UPDATE stock_movements SET approved_by = NULL WHERE approved_by = ?', [id]).catch(() => {});
    await pool.execute('UPDATE plant_health_logs SET checked_by = NULL WHERE checked_by = ?', [id]).catch(() => {});
