...
import type { ActivityLog, AdvancedAnalytics, Category, DashboardStats, Plant } from '../types';
...
interface StatCardProps {
  ...
  value: ReactNode;
  icon: LucideIcon;
  accent?: 'moss' | 'blue' | 'amber' | 'red';
  trend?: string;
  helpText?: string;
}

const StatCard = ({ label, value, icon: Icon, accent = 'moss', trend, helpText }: StatCardProps) => {
...
const DashboardHome = () => {
  ...
  const [lowStock, setLowStock] = useState<Plant[]>([]);
  const [categoryStats, setCategoryStats] = useState<Category[]>([]);
  const [activities, setActivities] = useState<ActivityLog[]>([]);
  const [healthSummary, setHealthSummary] = useState<{ health_status: string; count: number }[]>([]);
  ...
  const formatHealthLabel = (s: string) => s.replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase());
