...
const getOverview: RequestHandler = async (req, res) => {
  try {
    // Total plants count
    const [plantCount] = await pool.execute<RowDataPacket[]>('SELECT COUNT(*) as total FROM plants WHERE is_active = 1');

    // Total current stock
    const [stockTotal] = await pool.execute<RowDataPacket[]>('SELECT SUM(current_stock) as total FROM plants WHERE is_active = 1');

    // Low stock plants count
    const [lowStockCount] = await pool.execute<RowDataPacket[]>(
      'SELECT COUNT(*) as total FROM plants WHERE is_active = 1 AND current_stock <= min_stock_threshold'
    );

    // Plants by health status
    const [healthStatus] = await pool.execute<RowDataPacket[]>(
    ...
    const [activityCount] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) as total FROM activity_logs WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)`
    );

    const healthByStatus: Record<string, any> = {};
...
const getLowStock: RequestHandler = async (req, res) => {
  try {
...
const getCategoryStats: RequestHandler = async (req, res) => {
  try {
...
const getHealthSummary: RequestHandler = async (req, res) => {
  try {
...
const getAdvancedAnalytics: RequestHandler = async (req, res) => {
  try {
    // Stock movement trends (last 30 days)
    const [stockTrends] = await pool.execute<RowDataPacket[]>(
      `SELECT 
    ...
    const [categoryPerformance] = await pool.execute<RowDataPacket[]>(
      `SELECT 
        c.category_name,
        COUNT(p.plant_id) as plant_count,
    ...
    const [healthTrends] = await pool.execute<RowDataPacket[]>(
      `SELECT 
    ...
    const [userActivity] = await pool.execute<RowDataPacket[]>(
      `SELECT 
        u.username,
        u.role_id,
        r.role_name,
...
export { getOverview, getLowStock, getCategoryStats, getRecentActivities, getHealthSummary, getAdvancedAnalytics };