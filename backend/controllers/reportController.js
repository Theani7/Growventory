...
const exportHealthLogsCSV = async (req, res) => {
  try {
...
const getSummaryReport = async (req, res) => {
  try {
    ...
    const [plants] = await pool.execute('SELECT COUNT(*) as total FROM plants WHERE is_active = 1');
    const [stock] = await pool.execute('SELECT SUM(current_stock) as total FROM plants WHERE is_active = 1');
    const [lowStock] = await pool.execute('SELECT COUNT(*) as total FROM plants WHERE current_stock <= min_stock_threshold AND is_active = 1');
    const [categories] = await pool.execute('SELECT COUNT(*) as total FROM categories');
    const [users] = await pool.execute('SELECT COUNT(*) as total FROM users WHERE is_active = 1');
...
const generateStockMovementsPDF = async (req, res) => {
  try {
    const { plant_id, start_date, end_date } = req.query;
    let query = `
      SELECT sm.*, p.name as plant_name, u.username 
